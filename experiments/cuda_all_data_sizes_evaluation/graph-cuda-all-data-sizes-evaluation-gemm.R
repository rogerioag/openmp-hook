...
setwd("/dados/rogerio/USP/doutorado/prova-de-conceito/testes-prova-conceito/openmp-hook/experiments/cuda_all_data_sizes_evaluation/graph");

# Load data about OMP, CUDA, OMP_OFF.
data = read.csv("/dados/rogerio/USP/doutorado/prova-de-conceito/testes-prova-conceito/openmp-hook/experiments/cuda_all_data_sizes_evaluation/graph/gemm-09-08-2016-20-59-47-joined-processed-final.csv");
View(data)
...
test <-subset(cdata, version == "CUDA")
write.csv(test, file = "gemm-cuda-all-data-sizes-09-08-2016-20-59-47.csv")

#    exp     size_of_data   NI   NJ   NK  N  mean_orig mean_cuda      sd_orig    sd_cuda      se_orig
df_data = data.frame(x=cdata$NI, y=cdata$chunk_size, z=cdata$mean_orig, z_se=cdata$se_orig, z_sd=cdata$sd_orig, t=cdata$mean_plot_value, t_se=cdata$se_plot_value, t_sd=cdata$sd_plot_value, cat=cdata$NI)
...
pdf(filename="cuda-all-data-sizes-evaluation.pdf", width=1200, height=800);

p1 <- ggplot(df_plot, aes(x=x, fill=cat)) + 
  geom_bar(aes(y=t), stat="identity", position="dodge") +
  geom_errorbar(aes(ymin=t-t_sd, ymax=t+t_sd),
                size=.5,    # Thinner lines
                width=.4,
                position=position_dodge(.9)) + 
  geom_bar(aes(y=z), stat="identity", position="dodge") +
  geom_errorbar(aes(ymin=z-z_sd, ymax=z+z_sd),
                size=.5,    # Thinner lines
                width=.4,
                position=position_dodge(.9)) +
  xlab("Data Size") +
  ...
  ggtitle("CUDA all Data Sizes Analysis for gemm\n") +
...
multiplot(p1, cols=1)
dev.copy2pdf(file = "cuda-all-data-sizes-evaluation.pdf");
# dev.off ();
...
pdf(filename="evaluating-chunk_size-benchmark-gemm-data-large_dataset-num_threads-1-a-24-dynamic-chunk_size-32.pdf", width=1200, height=800);
...
multiplot(p2, cols=1)
dev.copy2pdf(file = "evaluating-chunk_size-benchmark-gemm-data-large_dataset-num_threads-1-a-24-dynamic-chunk_size-32.pdf");
# dev.off ();
...
multiplot(p3, cols=1)
dev.copy2pdf(file = "evaluating-chunk_size-benchmark-gemm-data-large_dataset-num_threads-1-a-24-dynamic-chunk_size-64.pdf");
# dev.off ();
...
pdf(filename="evaluating-chunk_size-benchmark-gemm-data-large_dataset-num_threads-1-a-24-dynamic-chunk_size-128.pdf", width=1200, height=800);
