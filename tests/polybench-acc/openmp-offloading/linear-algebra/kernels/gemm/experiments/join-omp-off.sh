            ...
            for num_threads in 1 2 4 8 10 12 16 18 20 22 24; do
                for omp_schedule in DYNAMIC; do
                    for chunk_size in 16 32 64; do
                        ...
                        echo "File: ${experiment_date}/data-${benchmark}-dataset-${size_of_data}-schedule-${omp_schedule}-chunk-${chunk_size}-threads-${num_threads}-offloading-gpu.csv"
                        cat ${INPUT}/${experiment_date}/data-${benchmark}-dataset-${size_of_data}-schedule-${omp_schedule}-chunk-${chunk_size}-threads-${num_threads}-offloading-gpu.csv >> ${OUTPUT}/${experiment_date}-joined.csv
                    done
                done
            done
        done
