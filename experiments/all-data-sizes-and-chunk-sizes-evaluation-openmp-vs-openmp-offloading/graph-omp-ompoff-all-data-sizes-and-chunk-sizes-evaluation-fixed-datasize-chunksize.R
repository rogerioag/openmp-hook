...
myLoadAndPrepareData<-function(p_experiment_name, p_benchmark_name, p_machine_name, p_csv_file_name)
{
  ...
  df_data <<- data.frame(x=cdata$num_threads, y=cdata$chunk_size, z=cdata$NI, w=cdata$mean_orig, w_se=cdata$se_orig, w_sd=cdata$sd_orig, t=cdata$mean_plot_value, t_se=cdata$se_plot_value, t_sd=cdata$sd_plot_value, off_wfbod = cdata$sum_work_finish_before_offload_decision, off_rodp = cdata$sum_reach_offload_decision_point, off_decoff = cdata$sum_decided_by_offloading, off_mkoff = cdata$sum_made_the_offloading, cat=cdata$version)
...
myFilterAndPlot<-function(p_experiment_name, p_benchmark_name, p_machine_name, p_machine_arch, p_machine_processor, p_machine_exp_id, p_data_size, p_chunk_size)
{
  graph_title <- paste0("OpenMP vs. OMP+OFF Analysis for ", p_benchmark_name)
  graph_parameters <- paste0("(Size of Data: ", p_data_size,", with chunk_size: ", p_chunk_size,")")
  
  graph_platform_details <- paste0("exp: ", p_experiment_name, ", machine: ", p_machine_exp_id, ", server: ", p_machine_name, ", processor: ", p_machine_processor ,", arch: ", p_machine_arch)
  
  plot.title <- paste0(graph_title," ", graph_parameters)
  plot.subtitle <- paste0(graph_platform_details)
  
  graph_file_name <- paste0("graph/", p_machine_name, "/", "graph-omp-vs-ompoff-", p_experiment_name,"-benchmark-",p_benchmark_name,"-machine-",p_machine_name,"-arch-",p_machine_arch,"-data_size-",p_data_size,"-chunk_size-",p_chunk_size,".pdf")
  
  dir.create(paste0("csv/", p_machine_name, "/generated-csv/"), showWarnings = FALSE)
  csv_file_name <- paste0("csv/", p_machine_name, "/generated-csv/", "data-omp-vs-ompoff-", p_experiment_name,"-benchmark-",p_benchmark_name,"-machine-",p_machine_name,"-arch-",p_machine_arch,"-data_size-",p_data_size,"-chunk_size-",p_chunk_size,".csv")
  
  # where num_threads = 1 and chunk_size = 64
  df_plot_num_threads_1_chunk_size_64 <- subset(df_data, (cat == "OMP+OFF" | cat == "OMP") & (y == p_chunk_size & z == p_data_size))
  ...
  p1 <- ggplot(df_plot_num_threads_1_chunk_size_64, aes(x=x, y=t, fill=cat)) + 
    geom_bar(stat="identity", position="dodge") +
    geom_errorbar(aes(x=x,ymin=t-t_sd, ymax=t+t_sd),
                  size=.5,    # Thinner lines
                  width=.4,
                  position=position_dodge(.9)) +
    geom_label(aes(label=ifelse(df_plot_num_threads_1_chunk_size_64$cat == "OMP+OFF", paste0("[", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_rodp)>0, "t", "f"), ", ", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_decoff)>0, "t", "f"), ", ", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_mkoff)>0, "t", "f"), "]"), NA), na.rm = TRUE, show.legend = FALSE, vjust="middle", hjust="left", check_overlap = FALSE), position = position_dodge(0.9), colour="gray") +
    # geom_label(aes(label=ifelse(df_plot_num_threads_1_chunk_size_64$cat == "OMP+OFF", paste0("[", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_rodp)>0, "T", "F"), ", ", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_decoff)>0, "T", "F"), ", ", ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_mkoff)>0, "T", "F"), "]"), NA), na.rm = TRUE, show.legend = FALSE, vjust=4.0), position = position_dodge(0.9)) +
    # geom_label(aes(label=ifelse(df_plot_num_threads_1_chunk_size_64$cat == "OMP+OFF", paste0("[", df_plot_num_threads_1_chunk_size_64$off_wfbod, ", ", df_plot_num_threads_1_chunk_size_64$off_rodp, ", ", df_plot_num_threads_1_chunk_size_64$off_decoff, ", ", df_plot_num_threads_1_chunk_size_64$off_mkoff, "]"), NA), na.rm = TRUE, show.legend = FALSE, vjust=4.0), position = position_dodge(0.9)) +
    # geom_label(aes(label=ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_mkoff)>0, ifelse(df_plot_num_threads_1_chunk_size_64$cat == "OMP+OFF","OFF", "OMP"), NA), na.rm = TRUE, show.legend = FALSE,                   vjust=4.0), position = position_dodge(0.9)) + 
    # geom_text(aes(label=ifelse(sign(df_plot_num_threads_1_chunk_size_64$off_mkoff)>0, "OFF", ""), vjust=-4.0), position = position_dodge()) + 
    # geom_text(aes(label = df_plot_num_threads_1_chunk_size_64$off_mkoff), position=position_dodge(-0.5, -1.0)) + 
    # geom_label(aes(label = df_plot_num_threads_1_chunk_size_64$off_mkoff), position=position_stack()) +
    # geom_point(aes(size = df_plot_num_threads_1_chunk_size_64$off_mkoff), position=position_dodge(0.9, 10)) +
...
machine_processor <- vector(mode="list", length=3)
names(machine_processor) <- c("pilipili2", "ragserver", "hppca-pacgpu-server01")
machine_processor[[1]] <- "Xeon E5-2630 v2"; 
machine_processor[[2]] <- "Core i7-870";
machine_processor[[3]] <- "Xeon E5-2630 v0"

machine_exp_id <- vector(mode="list", length=3)
names(machine_exp_id) <- c("pilipili2", "ragserver", "hppca-pacgpu-server01")
machine_exp_id[[1]] <- "M1"; 
machine_exp_id[[2]] <- "M2";
machine_exp_id[[3]] <- "M3"
...
data_size_configs <- c(32, 64, 128, 256, 512, 1024, 2048, 4096, 8192)
...
# myFilterAndPlot(experiment_name, benchmark_name, machine_name, machine_arch[machine_name], p_data_size = 1024, p_chunk_size = 64)

# Filtra e Plota os gráficos.
for (ds in data_size_configs) {
  for (cs in chunk_size_configs) {
    print(paste("Executing for: ", ds, ", ", cs))
    # myFilterAndPlot(p_experiment_name, p_benchmark_name, p_machine_name, p_machine_arch, p_machine_processor, p_machine_exp_id, p_data_size, p_chunk_size)
    myFilterAndPlot(experiment_name, benchmark_name, machine_name, machine_arch[machine_name], machine_processor[machine_name], machine_exp_id[machine_name], p_data_size = ds, p_chunk_size = cs)
  }  
}
...
# myFilterAndPlot(1, 32)
myFilterAndPlot(experiment_name, benchmark_name, machine_name, machine_arch[machine_name], machine_processor[machine_name], machine_exp_id[machine_name], p_data_size = 4096, p_chunk_size = 64)
