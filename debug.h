...
void timestamp()
{
 ...
 #define TRACE(fmt, args...)	do{fprintf(stderr, "[TRACE]: [%10s:%010lu] Thread [%lu] in %s(): " fmt, \
...
#define PRINT_FUNC_NAME fprintf(stderr, "TRACE-FUNC-NAME: [%10s:%010lu] Thread [%lu] is calling [%s()]\n",__FILE__, __LINE__, (unsigned long int) pthread_self(), __FUNCTION__)
#else
